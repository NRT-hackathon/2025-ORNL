...
def temporal_image_preprocessing(temporal_data, norm=0):
    """
    Preprocess temporal image data for the model.
    
    Parameters:
    temporal_data: Array of shape [samples, seq_len, height, width]
    norm: Whether to normalize the data
    
    Returns:
    Processed data of shape [samples, seq_len, height, width, 1]
    """
    # Add channel dimension
    processed_data = temporal_data.reshape(
        temporal_data.shape[0], temporal_data.shape[1], 
        temporal_data.shape[2], temporal_data.shape[3], 1
    )
    ...
    if norm != 0:
        ...
        for i in range(processed_data.shape[0]):
            ...
            sample = (sample - np.amin(sample)) / (np.amax(sample) - np.amin(sample) + 1e-10)
...
def model_temporal_conv3d(input_shape, nb_classes=2):
    """
    Creates a 3D convolutional model for temporal defect identification.
    
    Parameters:
    input_shape: Shape of input (seq_len, height, width, channels)
    ...
    # Output layers
    x = layers.Conv2D(nb_classes, (3, 3), activation='linear', padding='same')(x)
    x = layers.Conv2D(nb_classes, (1, 1), activation='linear', padding='same')(x)
    output = layers.Activation('softmax')(x)
    
    return models.Model(input_img, output)

def model_temporal_convlstm(input_shape, nb_classes=2):
    """
    Creates a ConvLSTM model for temporal defect identification.
    ...
    # ConvLSTM layers
    x = ConvLSTM2D(16, (3, 3), activation='relu', padding='same', return_sequences=True)(input_img)
    x = TimeDistributed(layers.MaxPooling2D((2, 2), padding='same'))(x)
    x = ConvLSTM2D(32, (3, 3), activation='relu', padding='same', return_sequences=False)(x)
...
def model_temporal_encoder_decoder(input_shape, nb_classes=2):
    """
    Creates an encoder-decoder model with temporal awareness for defect identification.
    ...
    Keras model
    """
    from keras.layers import TimeDistributed, Reshape, Flatten, Dense
    
    input_img = layers.Input(shape=input_shape)
    seq_len = input_shape[0]
    
    # Use TimeDistributed to apply same Conv2D to each frame
    x = TimeDistributed(layers.Conv2D(16, (3, 3), activation='relu', padding='same'))(input_img)
    x = TimeDistributed(layers.MaxPooling2D((2, 2), padding='same'))(x)
    x = TimeDistributed(layers.Conv2D(32, (3, 3), activation='relu', padding='same'))(x)
    x = TimeDistributed(layers.MaxPooling2D((2, 2), padding='same'))(x)
    
    # Get shape after pooling for later upsampling
    _, _, h, w, c = x.shape
...
# %%
def generate_temporal_windows(imgdata, window_size, window_step, seq_lengths, test_edge=None):
    """
    Generate temporally stacked windows for training and testing.
    
    Parameters:
    imgdata: Array of image frames [frames, height, width]
    ...
    test_edge: Which edge to use for test data ('top', 'bottom', 'left', 'right', or None)
    
    Returns:
    Dictionary containing training and test data for each sequence length
    """
    image_width = imgdata[0].shape[0]
    pos_mat = GenerateXYPos(window_size, window_step, image_width)
    
    # Separate test windows if needed
    test_indices = []
    train_indices = list(range(len(pos_mat)))
    
    if test_edge:
        if test_edge == 'top':
            test_indices = [i for i, pos in enumerate(pos_mat) if pos[0] < window_step]
        elif test_edge == 'bottom':
            test_indices = [i for i, pos in enumerate(pos_mat) if pos[0] > image_width - window_size - window_step]
        elif test_edge == 'left':
            test_indices = [i for i, pos in enumerate(pos_mat) if pos[1] < window_step]
        elif test_edge == 'right':
            test_indices = [i for i, pos in enumerate(pos_mat) if pos[1] > image_width - window_size - window_step]
        
        # Remove test indices from training
        train_indices = [i for i in train_indices if i not in test_indices]
    
    # Get the positions for training and testing
    train_pos = pos_mat[train_indices]
    test_pos = pos_mat[test_indices] if test_indices else np.array([])
    
    result = {}
    max_seq_len = max(seq_lengths)
    n_frames = len(imgdata)
    
    # Generate data for each sequence length
    for seq_len in seq_lengths:
        ...
        # Start from frame where we have enough history
        for frame_idx in range(max_seq_len-1, n_frames):
            # Generate label from current frame
            label_frame = frame_idx
            label_img = imgdata[label_frame]
            
            # Stack temporal windows for this sequence length
            for pos_idx, (x, y) in enumerate(train_pos):
                # Extract label window
                label_window = MakeWindow(label_img, x, y, window_size)
                
                # Create temporal stack
                seq_stack = []
                for seq_idx in range(seq_len):
                    # Get frame index, counting backward from current frame
                    temp_frame_idx = frame_idx - (seq_len - 1) + seq_idx
                    frame = imgdata[temp_frame_idx]
                    window = MakeWindow(frame, x, y, window_size)
                    seq_stack.append(window)
                
                # Convert to numpy array [seq_len, window_size, window_size]
                seq_stack = np.array(seq_stack)
                
                train_data.append(seq_stack)
                train_labels.append(label_window)
            
            # Do the same for test data if we have any
            if len(test_pos) > 0:
                for x, y in test_pos:
                    label_window = MakeWindow(label_img, x, y, window_size)
                    ...
                    test_data.append(seq_stack)
                    test_labels.append(label_window)
        
        # Convert to numpy arrays
        train_data = np.array(train_data)
        train_labels = np.array(train_labels)
        test_data = np.array(test_data) if len(test_data) > 0 else np.array([])
        test_labels = np.array(test_labels) if len(test_labels) > 0 else np.array([])
...
# %%
def train_temporal_models(imgdata, labelIm, seq_lengths, window_size=256, window_step=8, test_edge='top', model_type='conv3d'):
    """
    Train temporal models for each sequence length.
    
    Parameters:
    imgdata: Array of image frames
    labelIm: Array of label images
    seq_lengths: List of sequence lengths to generate
    window_size: Size of the spatial window
    window_step: Step size for sliding window
    test_edge: Which edge to use for test data ('top', 'bottom', 'left', 'right')
    model_type: Type of model to use ('conv3d', 'convlstm', 'encoder_decoder')
    
    Returns:
    Dictionary of trained models for each sequence length
    """
    nb_classes = 2  # Assuming binary classification
    
    # Generate temporal windows
    data_dict = generate_temporal_windows(imgdata, window_size, window_step, seq_lengths, test_edge)
    
    # For labels, we need to find corresponding points in labelIm
    for seq_len in seq_lengths:
        # Get the labels for training data
        train_labels = data_dict[seq_len]['train_labels']
        # Convert labels to one-hot encoded format
        train_labels = label_preprocessing(train_labels, nb_classes)
        data_dict[seq_len]['train_labels'] = train_labels
        
        # Do the same for test data if available
        if len(data_dict[seq_len]['test_labels']) > 0:
            test_labels = data_dict[seq_len]['test_labels']
            test_labels = label_preprocessing(test_labels, nb_classes)
            data_dict[seq_len]['test_labels'] = test_labels
    
    # Preprocess the temporal data
    for seq_len in seq_lengths:
        data_dict[seq_len]['train_data'] = temporal_image_preprocessing(data_dict[seq_len]['train_data'], norm=1)
        if len(data_dict[seq_len]['test_data']) > 0:
            data_dict[seq_len]['test_data'] = temporal_image_preprocessing(data_dict[seq_len]['test_data'], norm=1)
    
    # Train models for each sequence length
    models_dict = {}
    for seq_len in seq_lengths:
        print(f"Training model for sequence length {seq_len}")
        
        # Define input shape based on sequence length
        input_shape = data_dict[seq_len]['train_data'].shape[1:]  # (seq_len, height, width, channels)
        
        # Create model based on specified type
        if model_type == 'conv3d':
            model = model_temporal_conv3d(input_shape, nb_classes)
        elif model_type == 'convlstm':
            model = model_temporal_convlstm(input_shape, nb_classes)
        elif model_type == 'encoder_decoder':
            model = model_temporal_encoder_decoder(input_shape, nb_classes)
        else:
        ...
        X_train = data_dict[seq_len]['train_data']
        y_train = data_dict[seq_len]['train_labels']
        
        # Use test data as validation if available, otherwise split training data
        if len(data_dict[seq_len]['test_data']) > 0:
            X_val = data_dict[seq_len]['test_data']
            y_val = data_dict[seq_len]['test_labels']
        else:
            # Split train data into train and validation
            X_train, X_val, y_train, y_val = train_test_split(X_train, y_train, test_size=0.2, random_state=42)
        
        # Train model
        callback_tb = callbacks.TensorBoard(log_dir=f'/tmp/AtomGen_seq_{seq_len}', 
                                          histogram_freq=0,
                                          write_graph=True, 
                                          write_images=False)
        ...
        # Save model
        model.save(f"./DefectModel_seq_{seq_len}.h5")
        model.save_weights(f"./DefectModel_seq_{seq_len}.weights.h5")
        print(f'Saved model and weights for sequence length {seq_len} to disk.\n')
        
        # Store model in dictionary
        models_dict[seq_len] = model
    
    return models_dict, data_dict

# %%
def predictDefectsWithTemporalModel(imgdata, model, seq_len, target_size, nb_classes=2):
    """
    Predicts defects using a trained temporal model.
    
    Parameters:
    imgdata: Array of image frames
    model: Trained temporal model
    seq_len: Length of the temporal sequence
    target_size: Tuple of (height, width) for window size
    nb_classes: Number of classes in prediction
    
    Returns:
    Prediction map for the last frame
    """
    n_frames = len(imgdata)
    if n_frames < seq_len:
        raise ValueError(f"Not enough frames for sequence length {seq_len}")
    
    # Get dimensions of last frame
    last_frame = imgdata[-1]
    xs = last_frame.shape[0]
    ys = last_frame.shape[1]
    ...
    for i in range(nxp):
        for j in range(nyp):
            xstart = target_size[0] * i
            ystart = target_size[1] * j
            xend = target_size[0] * (i + 1)
            yend = target_size[1] * (j + 1)
            
            # Create temporal stack for this patch
            seq_stack = []
            for idx in range(seq_len):
                frame_idx = n_frames - seq_len + idx
                frame = imgdata[frame_idx]
                patch = frame[xstart:xend, ystart:yend]
                seq_stack.append(patch)
            
            # Convert to numpy array and add batch/channel dimensions
            seq_stack = np.array(seq_stack)
            seq_stack = seq_stack.reshape(1, seq_len, target_size[0], target_size[1], 1)
            
            # Normalize the patch
            seq_stack = (seq_stack - np.amin(seq_stack))/(np.amax(seq_stack) - np.amin(seq_stack) + 1e-10)
            
            # Get prediction
            pred = model.predict(seq_stack, verbose=0)
            
            # Store prediction in output array
            for c in range(nb_classes):
                classpred[c, xstart:xend, ystart:yend] = pred[0, :, :, c]
    
    return classpred

def predictDefectsWithTemporalModelOverlap(imgdata, model, seq_len, target_size, nb_classes=2):
    """
    Predicts defects using a trained temporal model with overlapping windows.
    ...
    for x_start in xpos_vec:
        for y_start in ypos_vec:
            x_end = x_start + window_size
            y_end = y_start + window_size
            ...
            # Convert to numpy array and add batch/channel dimensions
            seq_stack = np.array(seq_stack)
            seq_stack = seq_stack.reshape(1, seq_len, window_size, window_size, 1)
            ...
            # Accumulate predictions and counts
            for c in range(nb_classes):
                predictions[c, x_start:x_end, y_start:y_end] += pred[0, :, :, c]
            counts[x_start:x_end, y_start:y_end] += 1
    
    # Average predictions by count of overlapping windows
    for c in range(nb_classes):
        predictions[c] /= np.maximum(counts, 1)  # Avoid division by zero
    
    return predictions

def evaluate_temporal_models(models_dict, imgdata, target_size, softmax_threshold=0.8):
    """
    Evaluate all trained temporal models on the given image data.
    
    Parameters:
    models_dict: Dictionary of trained models keyed by sequence length
    imgdata: Array of image frames
    target_size: Tuple of (height, width) for the window size
    softmax_threshold: Threshold for defect detection
    ...
    eval_directory = './Figures/Evaluation/'
    if not os.path.exists(eval_directory):
        os.makedirs(eval_directory)
    
    # Get the last frame for comparison
    last_frame = imgdata[-1]
    
    # Evaluate each model
    for seq_len, model in models_dict.items():
        print(f"Evaluating model for sequence length {seq_len}")
        
        # Predict defects
        classpred = predictDefectsWithTemporalModelOverlap(imgdata, model, seq_len, target_size)
        
        # Extract defects
        thresh, defim, defcoord = extractDefectsOverlap(last_frame, classpred, softmax_threshold)
        
        # Visualize results
        fig = plt.figure(figsize=(10, 5))
        ax1 = fig.add_subplot(121)
        ax1.imshow(last_frame)
        ax1.set_title(f'Original (Last Frame)')
        ax1.axis('off')
        
        ax2 = fig.add_subplot(122)
        ax2.imshow(last_frame)
        ax2.imshow(classpred[1] > softmax_threshold, cmap='hot', alpha=0.5)
        ax2.set_title(f'Seq Length {seq_len} - Defect Prediction')
        ...
        for point in defcoord:
            ...
            p = patches.Rectangle((starty, startx), bbox*2, bbox*2, fill=False, edgecolor='red', lw=2)
            ax2.add_patch(p)
        
        ax2.axis('off')
        
        # Save figure
        fig.savefig(f"{eval_directory}eval_seq_{seq_len}.png")
        plt.close(fig)
        
        # Store results
        results[seq_len] = {
            'prediction': classpred,
            'defects_threshold': thresh,
            'defects_images': defim,
            'defects_coordinates': defcoord
        }
